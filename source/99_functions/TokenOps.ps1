...
Function New-AccessToken {
    ...
    if (!$ForceRefresh) {
        if ((Get-Date) -lt (Get-AccessToken).ExpiresOn ) {
            SayInfo "A valid access token is found in this session. If you want to force a new token, use the -ForceRefresh switch."
            return $null
        }
    }
